...
class DumpsysWindow :
    ...
    def current_app_size(self, currentDumpsys) :
        #app Size 미리 계산해놔야함
        display = ""
...
class DeviceUIInfo :
    ...
    def __init__(self) :
        self.devcie = ""
        self.clickable_list = list()
        self.clickable_tree = list()
        self.obj_group = list()

    def __append_clickable_tree(self, clickableTree, click) :
        u'''
        adb shell uiautomator dump /sdcard/vivi.xml
        cat /sdcard/vivi.xml
    ...
    def __get_clickable_tree(self, clickableList) :
        u'''
        y축 기준으로 트리를 구성
        ...
        for click in clickableList :
            #click.prints()
            clickableTree = self.__append_clickable_tree(clickableTree, click)
        #print "$" * 110
        return clickableTree

    def __order_clickable_tree(self, clickableTree) :
        u'''
    ...
    def prints_clickable_tree(self, clickableTree) :
        u'''
    ...
    def is_clickable_collision(self, clickableTree, x1, y1, x2, y2) :
        u'''
    ...
    def window_point_parsing(self, device="", fx=0, fy=0) :
        u'''
        현재 상태의 화면 분석
        ...
        temp_ui_auto_xml = list()
        for ui in uiautoXML :
            temp_ui_auto_xml.append(ui.decode("UTF-8").strip())
        uiautoXML = temp_ui_auto_xml
        ...
        while True :
            ...
            if obj.clickable == True :
                self.clickable_list.append(obj)
        ...
        for obj in objList :
            ...
            objCheck = False
            level = 0
            self.__obj_group_leveling(objGroup, obj)
        #objGroupPrints(objGroup)

    #Clickable 속성을 가진 OBJ를 y축 기준으로 정렬하여 넣고
    #X축 기준으로 재정렬
        self.clickable_tree = self.__get_clickable_tree(self.clickable_list)
        self.clickable_tree = self.__order_clickable_tree(self.clickable_tree)
        #self.prints_clickable_tree(self.clickable_tree)


        return (objGroup, self.clickable_tree)

    def __obj_group_leveling(self, objGroup, obj) :
        u'''
        ...
        if objGroup.x1 == obj.x1 and objGroup.y1 == obj.y1 and objGroup.x2 == obj.x2 and objGroup.y2 == obj.y2 :
            ...
            for _og in objGroup.groupDown :
                if self.__obj_group_leveling(_og, obj) == True :
                    return True
    ...
    def is_box_obj_name(self, objGroup, x1, y1, x2, y2) :
        u'''
        ...
        if objGroup.x1 == x1 and objGroup.y1 == y1 and objGroup.x2 == x2 and objGroup.y2 == y2 :
            ...
            for _og in objGroup.groupDown :
                tu = self.is_box_obj_name(_og, x1, y1, x2, y2)
                if tu[1] != None :
    ...
    def search_clickable_resource_id(self, resource_id) :
        u'''
        리소스 아이디를 받아 검색하여 해당 아이디와 동일한 이름의 리소스 정보를 반환
        :resource_id: 검색할 리소스 id
        return: resource_id에 맞는 리소스 정보를 반환
        '''
        for clickable in self.clickable_list :
            if clickable.resource_id == resource_id :
                return clickable

    def search_obj_name(self, objGroup, cNames, level) :
        u"""
        줄기(objGroup)에 포함된 모든 잎(obj)의 이름을 저장한 cNames에 해당하는 가장 마지막 obj를 찾기 위한 함수
        :param objGroup:
        :param cNames: is_box_obj_name 함수를 통하여 선택된 오브젝트까지의 줄기(ObjGroup)와 잎(Obj) 정보를 입력
        ...
        for _og in objGroup.groupDown :
            obj = self.search_obj_name(_og, cNames, level+1)
            if obj != None :
                return obj
        return None
    ...
    def obj_group_prints(self, objGroup) :
        u'''
        objGroup의 모든 정보를 출력
        :param objGroup:
        :return:
        ...
        for _og in objGroup.groupDown :
            self.obj_group_prints(_og)


if __name__ == "__main__":
    device_list = DeviceList()
    device_ui_info = DeviceUIInfo()
    print (device_list.devices)
    for device in device_list.devices :
        ...
        #device_ui_info.prints_clickable_tree(device_ui_info.clickable_tree)
        info = device_ui_info.search_clickable_resource_id("com.android.packageinstaller:id/ok_button")
...
def is_clickable_collision_name(clickableTree, x1, y1, x2, y2) :
    u'''
